...
main() {
    ...
    echo -e "${BLUE}Before running the image sync script, you need to manually login to the CI cluster:${NC}"
    echo ""
    echo "   1. Get your API token from: https://console-openshift-console.apps.ci.l2s4.p1.openshiftapps.com"
    echo "   2. Login to CI cluster:"
    echo "      oc login --token=<YOUR_TOKEN> --server=https://api.ci.l2s4.p1.openshiftapps.com:6443 --insecure-skip-tls-verify=true"
    echo "   3. Verify login:"
    echo "      oc whoami"
    echo ""
    echo -e "${BLUE}📝 Next steps:${NC}"
    echo "1. Complete manual oc login (see above)"
    echo "2. Run: ./05-sync-images.sh to sync OpenShift images from CI cluster"
    echo "3. Use: ~/registry-utils.sh to manage registry"
    echo ""
